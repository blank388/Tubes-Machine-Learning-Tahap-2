...
numerik = ['Premi','Lama_Berlangganan','Umur','Kode_Daerah','Kanal_Penjualan', 
          'Jenis_Kelamin', 'SIM', 'Kendaraan_Rusak', 'Sudah_Asuransi', 'Umur_Kendaraan']

trainDf[numerik] = scale.fit_transform(trainDf[numerik].values)
print(trainDf.head())

#Normalisasi Data test agar serupa dengan data train(kecuali kolom tertarik tidak di normalisasi)
print(testDf.head())
testDf[numerik] = scale.transform(testDf[numerik].values)
print(testDf.head())
...
# trainDf.to_csv('kendaraan_train_cleanforclassification_withoutscalling.csv', index=False)
# testDf.to_csv('kendaraan_test_cleanforclassification_withoutscalling.csv', index=False)
